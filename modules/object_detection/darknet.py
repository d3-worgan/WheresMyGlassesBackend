...
if os.name == "nt":
    ...
    # Load the environment path variables
    os.environ['PATH'] = cwd + ';' + os.environ['PATH']
    # print(os.environ['PATH'])
    
    # winGPUdll = r"C:\darknet-master\build\darknet\x64\yolo_cpp_nogpu.dll"
    ...
    # Load either the GPU dll or no GPU dll
    try:
        # Validating which version to load
        try:
            # Check for force_cpu env variable
        ...
        elif os.path.exists(winGPUdll):
            print("It definatly exists...")

        # Passed validation so load the GPU dll
        print("Loading lib " + winGPUdll)
...
def detect(net, meta, image, thresh=.5, hier_thresh=.5, nms=.45):
    ...
    Performs detection
    :param net:
    :param meta: Class names?
    :param image: Must be re-sized before here
    :param thresh:
    :param hier_thresh:
    :param nms:
    :return:
    ...
    # init
    num_detections = c_int(0)
    pnum = pointer(num_detections)
    i, c, h, w = image.shape
    ...
    if nms:
        do_nms_obj(detections, num_detections, meta.classes, nms)

    # Extract information from detections
    res = []
    for detection in range(num_detections):
        for class_index in range(meta.classes):
            if detections[detection].prob[class_index] > 0:
                b = detections[detection].bbox
                res.append((class_index, detections[detection].prob[class_index], (b.x, b.y, b.w, b.h)))
...
def detect1(net, meta, image, thresh=.5, hier_thresh=.5, nms=.45, debug= False):
    """
